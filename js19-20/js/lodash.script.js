$(function() {
  ...
  var skills = _(array).flatMap('skills').sortBy().uniq().value(); 
  var name = _(array).flatMap('name').sortBy('friends').value();
  var friends = _(array).flatMap('friends').flatMap('name').uniq(array).value();


  console.log('Array skills: ', skills);
  console.log('Array names: ', name);
